...
! function($) {
    ...
    $.fn.centerAround = function(button, settings) {
        /* _dx: 733
         _dy: 424*/
        var offset,
            width,
            ...
            buttonY,
            objectOffset;
        //console.log(button.model.attributes.type);
        if (button.model.attributes.type != 'link' &&  !(button.model instanceof joint.dia.Link)) {
            offset = button.$el.offset();
            width = button.model.attributes.size.width;
            height = button.model.attributes.size.height;
            buttonX = (offset.left - $(document).scrollLeft()) + width / 2;
            buttonY = (offset.top - $(document).scrollTop()) + height / 2;
            objectOffset = this.offset();
            $('.svg-edit').css("position", "fixed");
            $('.svg-edit').css('top', settings.evts.clientY - ($('.svg-edit').outerHeight() / 2) + 'px');
            $('.svg-edit').css('left', settings.evts.clientX - ($('.svg-edit').outerHeight() / 2) + 'px');
            this.css('position', 'fixed');
            this.css('top', settings.evts.clientY - (this.outerHeight() / 2) + 'px');
            this.css('left', settings.evts.clientX - (this.outerHeight() / 2) + 'px');
            return this;


        } else {
            //console.log(settings.evts);
            offset = V(button.$el[0]).bbox(true);
            width = offset.width;
            height = offset.height;
            buttonX = (offset.x - $(document).scrollLeft()) + width / 2;
            buttonY = (offset.y - $(document).scrollTop()) + height / 2;
            objectOffset = this.offset();
            $('.svg-edit').css('position', 'fixed');
            $('.svg-edit').css('top', settings.evts.clientY - ($('.svg-edit').outerHeight() / 2) + 'px');
            $('.svg-edit').css('left', settings.evts.clientX - ($('.svg-edit').outerHeight() / 2) + 'px');
    ...
    $.fn.flyIn = function(el, button, width, height, angle, step, radius, settings) {
        ...
        this.each(function(index) {
            ...
            var x = Math.round(width / 2 + radius * Math.cos(angle) - $(this).find("span").outerWidth() / 2),
                y = Math.round(height / 2 + radius * Math.sin(angle) - $(this).find("span").outerHeight() / 2 - parseInt(el.css('border-width')) / 2 - 15);
            $(this).css({ /*animateRotate(360).*/
                position: 'absolute',
    ...
    $.fn.fadeInIcon = function(el, button, width, height, angle, step, radius, settings) {
        ...
        this.each(function(index) {
            angle = (settings.angle[0] + (step * index)) * (Math.PI / 180);
            var x = Math.round(width / 2 + radius * Math.cos(angle) - $(this).find("span").outerWidth() / 2),
                y = Math.round(height / 2 + radius * Math.sin(angle) - $(this).find("span").outerHeight() / 2);
            $(this).css({
                position: 'absolute',
                left: x + 'px',
                top: y + 'px',
    ...
    $.fn.wheelmenu = function(options) {
        ...
        var button = settings.cell;
        //var el = $($(this).attr("data-href"));
        var el = $('#wheel');
        el.addClass("wheel");

        /*button.$el.css("opacity", 1).animate({
            opacity: 1
        })
*/
