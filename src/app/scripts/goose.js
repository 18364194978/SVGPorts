...
(function($, joint, vE, _, ROOF, GFC, bootbox) {
  ...
  $e.on('DelVtLink', function(vtObj) {
    bootbox.dialog({
      message: '确定要删除这条虚拟回路连接?',
      title: '删除',
      buttons: {
        yes: {
          label: '确定',
          ...
          callback: function() {
            let arrlink = [];
            arrlink.push($(vtObj).attr('data-id'));
            var delVirLink = ROOF.virlink.DelVirLink;
            delVirLink(arrlink, function(objfgj) {
              if (objfgj.status) {
                $('[data-id=' + $(vtObj).attr('data-iid') + ']').removeClass('list-group-item-warning');
                $('[data-id=' + $(vtObj).attr('data-oid') + ']').removeClass('list-group-item-warning');
                let xhd = $(vtObj).parents('.vlink-trs').index() + 1;
                $.each($(vtObj).parents('.vlink-trs').nextAll('.vlink-trs'), function(indele, eledat) {
                  eledat.toString();
                  $(vtObj).parents('.vlink-trs').nextAll('.vlink-trs').eq(indele).find('.vlink-xh').text(xhd);
                  xhd++;
                });
                $(vtObj).parents('.vlink-trs').remove();
                isEdit = 1;
              } else {
                GFC.showError(objfgj.err_msg);
              }
            });
            this.modal('hide');
          }
  ...
  $e.on('rerelink', function(ROOF, centerIed, sendDevName, dtd) {
    virlink.GetSvgSigInfoByFlowId(dtd.Guid, function(obj) {
      if (obj.status) {
        var jsonInfo = $.parseJSON(obj.json_info);
        ...
        $('.rm').off('dblclick').on('dblclick', function() { //1111111
          var that = this;
          var res = false;
          $('.placeloading').remove();
          if ($(that).parents('.vlink-trs').hasClass('cli')) {
            $(that).parents('.vlink-trs').removeClass('cli');
            $('#relinkV').hide();
            $('#linkV').show();
            res = true;
          } else {
            $('#Vlinklists').find('.cli').removeClass('cli');
            $(that).parents('.vlink-trs').addClass('cli');
            $('#relinkV').show();
            $('#linkV').hide();
          }
          if (centerIed === sendDevName) {
            if (res === false) {
              if ($(that).attr('data-type') === 'OUT') {
                $('.right-vport-list').append('<div class="placeloading"></div>');
              } else {
                $('.left-vport-list').append('<div class="placeloading"></div>')
              }
            }
            let arrlink = [];
            arrlink.push($(that).attr('removeData-id'));
            var delVirLink = ROOF.virlink.DelVirLink;
            var addVirLink = ROOF.virlink.AddVirLink;
            $('#relinkV').off('click').on('click', function() {
              var sendPortArray = [];
              var recvPortArray = [];
              if ($(that).attr('data-type') === 'OUT') {
                var startAr = $('#sendlist').find('.active');
                var startAt = $('#sendlist').find('.active').attr('data-id');
                var startNa = $('#sendlist').find('.active').attr('data-sigtype')
                var endAt = $(that).attr('data-iid');
                if (startAr.length !== 1) {
                  GFC.showError('请确保选中一个虚端子');
                  console.log('1');
                  return;
                }

                if ($('#sendlist').find('.active').attr('data-stype') !== $('#recvlist').find('[data-id="' + endAt + '"]').attr('data-stype')) {
                  GFC.showError('请确保两侧虚端子类型相同');
                  return;
                }
                delVirLink(arrlink, function(objfgj) {
                  if (objfgj.status) {
                    $('[data-id=' + $(that).attr('data-iid') + ']').removeClass('list-group-item-warning');
                    $('[data-id=' + $(that).attr('data-oid') + ']').removeClass('list-group-item-warning');
                    isEdit = 1;
                  } else {
                    GFC.showError(objfgj.err_msg);
                  }
                ...
                addVirLink(llarr, function(dlstj) {
                  ...
                  if (dlstj.status) {
                    ...
                    var inid = $(that).attr('data-iid');
                    $(that).parents('.vlink-trs').find('[data-id=' + inid + ']').attr('data-oid', ouddd.Guid);
                    $(that).parents('.vlink-trs').removeClass('autocss');
                    $(that).parents('.vlink-trs').find('#img').addClass('hide');
                    $(that).parents('.vlink-trs').find('[data-id=' + inid + ']').attr('removeData-id', dlstj.virlink_list[0].Guid)
                    $(that).parents('.vlink-trs').find('#delFlag').attr('data-id', dlstj.virlink_list[0].Guid);
                    $(that).parents('.vlink-trs').find('#delFlag').attr('data-oid', ouddd.Guid);
                    $(that).attr('data-id', ouddd.Guid);
                    $(that).parents('.vlink-trs').find('#ouName').attr('title', ouddd.ProoutsigBriefname);
                    $(that).parents('.vlink-trs').find('#ouName').text(ouddd.ProoutsigBriefname);
                    $(that).attr('removeData-id', dlstj.virlink_list[0].Guid);
                    $(that).attr('data-oid', ouddd.Guid);
                    $(that).attr('title', startNa);
                    $(that).text(startNa);
                    $('.removeVlink').off('click').on('click', function() {
                      let that = this;
                      $e.emit('DelVtLink', that);
                ...
                var endAr = $('#recvlist').find('.active');
                var startAt = $(that).attr('data-oid');
                var endAt = $('#recvlist').find('.active').attr('data-id');
                var endNa = $('#recvlist').find('.active').attr('data-sigtype');
                if (endAr.length !== 1) {
                  GFC.showError('请确保选中一个虚端子');
                  console.log('2');
                  return;
                }
                if ($('#sendlist').find('[data-id="' + startAt + '"]').attr('data-stype') !== $('#recvlist').find('.active').attr('data-stype')) {
                  GFC.showError('请确保两侧虚端子类型相同');
                  return;
                }
                delVirLink(arrlink, function(objfgj) {
                ...
                addVirLink(llarr, function(dlstj) {
                  ...
                  if (dlstj.status) {
                    ...
                    var ouid = $(that).attr('data-oid');
                    $(that).parents('.vlink-trs').find('[data-id=' + ouid + ']').attr('data-iid', inddd.Guid);
                    $(that).parents('.vlink-trs').removeClass('autocss');
                    $(that).parents('.vlink-trs').find('#img').addClass('hide');
                    $(that).parents('.vlink-trs').find('[data-id=' + ouid + ']').attr('removeData-id', dlstj.virlink_list[0].Guid)
                    $(that).parents('.vlink-trs').find('#delFlag').attr('data-id', dlstj.virlink_list[0].Guid);
                    $(that).parents('.vlink-trs').find('#delFlag').attr('data-iid', inddd.Guid);
                    $(that).parents('.vlink-trs').find('#inName').attr('title', inddd.ProinsigBriefname);
                    $(that).parents('.vlink-trs').find('#inName').text(inddd.ProinsigBriefname);
                    $(that).attr('data-id', inddd.Guid);
                    $(that).attr('removeData-id', dlstj.virlink_list[0].Guid);
                    $(that).attr('data-iid', inddd.Guid);
                    $(that).attr('title', endNa);
                    $(that).text(endNa);
                    $('.removeVlink').off('click').on('click', function() {
                      let that = this;
                      $e.emit('DelVtLink', that);
            ...
            $('#relinkV').off('click').on('click', function() {
              ...
              if ($(that).attr('data-type') === 'OUT') {
                var startAr = $('#sendlist').find('.active');
                var endAt = $('#sendlist').find('.active').attr('data-id');
                var endNa = $('#sendlist').find('.active').attr('data-sigtype');
                var startAt = $(that).attr('data-oid');
                if (startAr.length !== 1) {
                  GFC.showError('请确保选中一个虚端子');
                  console.log('3');
                ...
                if ($('#sendlist').find('.active').attr('data-stype') !== $('#recvlist').find('[data-id="' + startAt + '"]').attr('data-stype')) {
                  GFC.showError('请确保两侧虚端子类型相同');
                  return;
                }
                delVirLink(arrlink, function(objfgj) {
                ...
                var endAr = $('#recvlist').find('.active');
                var endAt = $(that).attr('data-iid');
                var startAt = $('#recvlist').find('.active').attr('data-id');
                var startNa = $('#recvlist').find('.active').attr('data-sigtype');
                if (endAr.length !== 1) {
                  GFC.showError('请确保选中一个虚端子');
                  console.log('4');
                  return;
                }
                if ($('#sendlist').find('[data-id="' + endAt + '"]').attr('data-stype') !== $('#recvlist').find('.active').attr('data-stype')) {
                  GFC.showError('请确保两侧虚端子类型相同');
                  return;
                }
                delVirLink(arrlink, function(objfgj) {
  ...
  //               $('[data-id=' + $(that).attr('data-iid') + ']').removeClass('list-group-item-warning');
  //               $('[data-id=' + $(that).attr('data-oid') + ']').removeClass('list-group-item-warning');
  //               let xhd = $(that).parents('.vlink-trs').index() + 1;
  //               $.each($(that).parents('.vlink-trs').nextAll('.vlink-trs'), function(indele, eledat) {
  //                 eledat.toString();
  //                 $(that).parents('.vlink-trs').nextAll('.vlink-trs').eq(indele).find('.vlink-xh').text(xhd);
  //                 xhd++;
  //               });
  //               $(that).parents('.vlink-trs').remove();
  //               isEdit = 1;
  //             } else {
  //               GFC.showError(objfgj.err_msg);
  //             }
  ...
  //                                                         <div class="vlink-xh tk" style="flex: 0.15;">${$('#Vlinklists').find('.vlink-trs').length + 1}</div>
  //                                                         <div class="tk hide" style="flex: 1.5;"><span title="${dtd.sendBG.ProdevName}" class="no-link-style">${dtd.sendBG.ProdevName}</span></div>
  //                                                         <div class="tk" style="flex: 1.5;"><span data-id="${ouddd.Guid}" removeData-id ="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" data-type ="OUT" class = "rm" style="cursor: pointer;" title="${ouddd.ProoutsigCustomname}" class="no-link-style">${ouddd.ProoutsigCustomname}</span></div>
  //                                                         <div class="tk" style="flex: 1.25;"><span title="${ouddd.ProoutsigBriefname}" class="no-link-style">${ouddd.ProoutsigBriefname}</span></div>
  //                                                         <div class="tk" style="flex: 0.75;"><span id ="delFlag" data-id="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" class="glyphicon glyphicon-remove-circle removeVlink"></span></div>
  //                                                         <div class="tk hide" style="flex: 1.5;"><span title="${dtd.recvBG.ProdevName}" class="no-link-style">${dtd.recvBG.ProdevName}</span></div>
  //                                                         <div class="tk" style="flex: 1.5;"><span data-id="${inddd.Guid}" removeData-id ="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" data-type ="IN" class = "rm" style="cursor: pointer;" title="${inddd.ProinsigCustomname}" class="no-link-style">${inddd.ProinsigCustomname}</span></div>
  //                                                         <div class="tk" style="flex: 1.25;"><span title="${inddd.ProinsigBriefname}" class="no-link-style">${inddd.ProinsigBriefname}</span></div>
  //                                                         </div>`.trim();
  //               } else {
  //                 vlinkStr = `<div class="vlink-trs">
  ...
  //                                                         <div class="vlink-xh tk" style="flex: 0.15;">${$('#Vlinklists').find('.vlink-trs').length + 1}</div>
  //                                                         <div class="tk hide" style="flex: 1.5;"><span title="${dtd.sendBG.ProdevName}" class="no-link-style">${dtd.sendBG.ProdevName}</span></div>
  //                                                         <div class="tk" style="flex: 1.5;"><span data-id="${inddd.Guid}" removeData-id ="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" data-type ="OUT" class = "rm" style="cursor: pointer;" title="${inddd.ProinsigCustomname}" class="no-link-style">${inddd.ProinsigCustomname}</span></div>
  //                                                         <div class="tk" style="flex: 1.25;"><span title="${inddd.ProinsigBriefname}" class="no-link-style">${inddd.ProinsigBriefname}</span></div>
  //                                                         <div class="tk" style="flex: 0.75;"><span id ="delFlag" data-id="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" class="glyphicon glyphicon-remove-circle removeVlink"></span></div>
  //                                                         <div class="tk hide" style="flex: 1.5;"><span title="${dtd.recvBG.ProdevName}" class="no-link-style">${dtd.recvBG.ProdevName}</span></div>
  //                                                         <div class="tk" style="flex: 1.5;"><span data-id="${ouddd.Guid}" removeData-id ="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" data-type ="OUT" class = "rm" style="cursor: pointer;" title="${ouddd.ProoutsigCustomname}" class="no-link-style">${ouddd.ProoutsigCustomname}</span></div>
  ...
  //                                                         <div class="tk" style="flex: 0.75;"><span id ="delFlag" data-id="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" class="glyphicon glyphicon-remove-circle removeVlink"></span></div>
  //                                                         <div class="tk hide" style="flex: 1.5;"><span title="${dtd.recvBG.ProdevName}" class="no-link-style">${dtd.recvBG.ProdevName}</span></div>
  //                                                         <div class="tk" style="flex: 1.5;"><span data-id="${ouddd.Guid}" removeData-id ="${dlstj.virlink_list[0].Guid}" data-oid="${ouddd.Guid}" data-iid="${inddd.Guid}" data-type ="IN" class = "rm" style="cursor: pointer;" title="${ouddd.ProoutsigCustomname}" class="no-link-style">${ouddd.ProoutsigCustomname}</span></div>
  //                                                         <div class="tk" style="flex: 1.25;"><span title="${ouddd.ProoutsigBriefname}" class="no-link-style">${ouddd.ProoutsigBriefname}</span></div>
  //                                                         </div>`.trim();
  //               }
  //               $('#Vlinklists').append(vlinkStr);
